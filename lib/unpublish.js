...
class Unpublish extends BaseCommand {
  ...
  /* istanbul ignore next - see test/lib/load-all-commands.js */
  static get params () {
    return ['dry-run', 'force', 'workspace', 'workspaces', 'include-workspace-root']
  }
  ...
  async unpublishWorkspaces (args, filters) {
    ...
    if (this.npm.config.get('include-workspace-root')) {
      const pkgPath = path.join(this.npm.localPrefix, 'package.json')
      const manifest = await readJson(pkgPath)
      workspaceNames.unshift(manifest.name)
    }
